...
**从左到右不断交换相邻逆序的元素，在一轮的循环之后，可以让未排序的 "最大元素" 上浮到右侧。**
...
<details><summary>Java冒泡排序单向自优化</summary><pre><code> 
//1、基础的 减i 优化
...
public static void bubboSort(int[] a) {
    ...
    int sortBorder = a.length - 1;//无序数列的边界，每次比较只需要比到这里为止
    for (int i = 0; i < a.length - 1; i++) {//外边界不重要了，基本都会提前退出
...
<details><summary>Java双向冒泡自优化</summary><pre><code> 
//双向排序2   基于1的优化, 自己写的，检验没问题
private static void Sort(int[] arr, int len) {
    int temp;
    int rightTemBor = 0;//右边临时边界
    int rightRealBor = arr.length - 1;//右边真实边界
    int leftTemBor = 0;
    int leftRealBor = 0;
