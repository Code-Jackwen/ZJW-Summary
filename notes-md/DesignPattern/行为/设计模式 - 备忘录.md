...
备忘录 Memento 实际上有两个接口：

一个是提供给 Caretaker 的窄接口，它只能将备忘录传递给其它对象；

一个是提供给 Originator 的宽接口，允许它访问到先前状态所需的所有数据。
...
以下实现了一个简单**计算器程序**，可以输入两个值，然后计算这两个值的和。
...
public class CalculatorImp implements Calculator {
    ...
    public PreviousCalculationToCareTaker backupLastCalculation() {
        // create a memento object used for restoring two numbers	恢复
