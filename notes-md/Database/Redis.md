...
哨兵模式是redis`高可用`的实现方式之一。简单理解为对多个 redis 节点监视的系统。
...
- 哨兵会通过`命令连接`每10s发送一次`INFO`命令，通过`INFO命令`，主节点会返回自己的run_id和自己的`从节点信息`。
-  通过`命令连接`向服务器的`_sentinel:hello`频道发送一条消息，通过`订阅连接`对服务器的`_sentinel:hello`频道做了监听。
